...
//!
//! SputnikVM works on two different levels. It can directly handle a
//! transaction, or a normal Ethereum execution context. To interact with the
//! virtual machine, you usually only need to work with the
//! [VM](trait.VM.html) trait.
//!
//! ### Lifecycle
//!
//! A VM can be started given a `Transaction` (or `Context`) and a
//! `BlockHeader`. The user can then `fire` or `step` to run it. `fire` runs
//! through all possible steps until it cannot continue, while `step` only
//! runs one step. These functions will only fail if they need information
//! (either accounts in the current block, or block hashes of previous blocks)
//! provided by `RequireError`. If failure occurs, use the function
//! `commit_account` and `commit_blockhash` to commit that information to the
//! VM, and `fire` or `step` again until it succeeds. The current VM status
//! can always be obtained using the `status` function.
//!
//! ### Patch
//!
//! Every VM is associated with a `Patch`. This patch tells the VM which
//! Ethereum network and which hard fork it is on. You will need to specify
//! the patch as the type parameter. To interact with multiple `Patch`es at
//! the same time, the use of trait objects is recommended.
//!
//! ```rust
//! extern crate bigint;
//! extern crate sputnikvm;
//!
//! use sputnikvm::{EmbeddedByzantiumPatch, VMTestPatch, HeaderParams,
//!     ValidTransaction, TransactionAction, SeqTransactionVM};
...
//!     };
//!
//!     let _vm = if block_number < 500 {
//!         SeqTransactionVM::<VMTestPatch>::new(transaction, header);
//!     } else {
//!         SeqTransactionVM::<EmbeddedByzantiumPatch>::new(transaction, header);
//!     };
//! }
//! ```
//!
...
//! Usually, you use the sequential memory module, for which we have a
//! definition called `SeqTransactionVM`. A call to `TransactionVM::new`
//! requires that the transaction be valid (according to the rules for an
//! Ethereum transaction). For an invalid transaction, it is possible for the
//! VM to panic. If you want to handle untrusted transactions, you should use
//! `SeqTransactionVM::new_untrusted` which will return an error if the
//! transaction is invalid.
//!
//! ### Context Execution
//!
//! To start a VM on the Context level, use the `ContextVM` struct. Usually,
//! you will use the sequential memory module, for which we have a definition
//! called `SeqContextVM`. Context execution, like other EVM implementations,
//! will not handle transaction-level gas reductions.
